...
class ArticleController extends PublicController
{
        ...
        if (IS_POST) {
			...
			if ($id > 0) {
				if (isset($data['submit'])) {
					...
					$this->success(L('add_success'), U('Article/index'));
				}
    ...
    public function update()
    {
        if (IS_POST) {
            ...
            if ($result !== false) {
				action_log('update_article', 'Article', $data['id'], $adminid);
                $this->success(L('edit_success'), U('Article/index'));
            } else {
