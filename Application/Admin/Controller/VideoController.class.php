...
class VideoController extends PublicController
{
        ...
        if (IS_POST) {
			...
			if ($id > 0) {
				if (isset($data['submit'])) {
					action_log('add_article', 'Video', $id, $adminid);
					$this->success(L('add_success'), U('Video/index'));
				}
    ...
    public function delete()
    {
        ...
        if (M('Video')->where(array('id' => $id, 'status' => array('eq' , 0)))->delete()) {
    ...
    public function edit()
    {
        ...
        $detail = M('Video')->where(array('id' => $id, 'status' => array('eq' , 1)))->find();
		if (!$detail) {
    ...
    public function update()
    {
        if (IS_POST) {
            ...
            $result = M('Video')->where(array('id' => $data['id']))->save($data);
            if ($result !== false) {
				action_log('update_article', 'Video', $data['id'], $adminid);
                $this->success(L('edit_success'), U('Video/index'));
            } else {
	...
	*/
    public function sortBatch()
    {
        if (IS_POST) {
            ...
            foreach ($sort_order as $key => $val) {
                M('Video')->where(array('id' => $key))->save(array('sort' => $val));
            }
	...
	public function status()
    {
        ...
        $status = $status ? 0 : 1;
       if (M('Video')->where(array('id'=>$id))->save(array('status' => $status, 'updatetime' => NOW_TIME))) {
	...
	public function clear(){
        $res = M('Video')->where(array('status' => array('eq' , 0)))->delete();
        if($res !== false){
