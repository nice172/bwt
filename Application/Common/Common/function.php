...
function get_user_avatar($userid){
	...
	if(empty($headpic)){
		$headpic=__ROOT__.'/Public/Home/Images/avatar.jpg';
	}else{
		$headpic=__ROOT__.$headpic;
	}
	return $headpic;
}
...
//查询栏目下子栏目的数量
function countnum($catid, $sql='Category', $pid = 'pid'){
	$csql =  M($sql)->field('count(*) as c_num')->where(array($pid=>$catid))->find();
	return $csql['c_num'];
}
...
//查询表数据数量
function num($sql){
	$csql =  M($sql)->field('count(*) as c_num')->find();
	return $csql['c_num'];
}

//查询表数据启用数量
function counum($sql){
	$csql =  M($sql)->field('count(*) as c_num')->where(array('status' => 1))->find();
	return $csql['c_num'];
}

function counum_article($sql,$catid){
	$info = $sql->field('count(*) as c_num')->where('catid in ("'.$catid.'")  and status = 1')->find();
	return $info['c_num'];
}
...
function  catid_str($catid,$model = 1){
	    $cate= M('Category');
		$list=$cate->where(array('ismodel' => array('eq', $model), 'ishidden' => array('eq', 0)))->select();
...
function get_catname($catid,$title,$c = 'catid'){
	$info = M('Category')->where(array($c=>$catid))->getField($title);
	return $info;
}

function get_article($catid,$title,$sql = 'Article'){
	$info = M($sql)->where(array('id'=>$catid))->getField($title);
	return $info;
}

function top_article($title,$sql,$catid){
	$info = $sql->where('catid in ("'.$catid.'")  and status = 1')->order('sort desc,id desc')->limit(1)->getField($title);
	return $info;
}
...
function getpage($count, $pagesize = 5) {
    ...
    $p->setConfig('prev', '&lt;');
    $p->setConfig('next', '&gt;');
    $p->setConfig('last', '&raquo;');
    $p->setConfig('first', '&laquo;');
 ...
 * @return string
 */
function unsafe_replace($string) {
...
function catpos($catid, $url, $symbol='<span>&gt</span>'){
		$cate=M('Category');
		$list=$cate->select();
		$list=getParents($list,$catid);
		$str='';
		foreach($list as $v){
			$str.='<a href='.U($url).'>'.$v['catname'].'</a>'.$symbol;
		}
		$str= '<a href='.U('index/index').'>首页</a>'.$symbol.$str;	
		return $str;
...
//第三方登录
function getUserid() {
    $s_userid = session('admin_user.userid');
    if ($s_userid == '') {
        $s_userid = cookie('userid');
        if ($s_userid > 0) {
            $info = M("User")->field("username")->where("userid= '" . $s_userid . "'")->find();
