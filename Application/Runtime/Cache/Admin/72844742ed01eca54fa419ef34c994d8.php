    ...
    <div class="content">
        <div class="Config_nav">
            <ul>
                ...
                <li><a class="on" href="<?php echo U('Article/recycle');?>">回收站</a></li>
            </ul>
        </div>
        <form action="<?php echo U('Article/clear');?>" name="myform" id="myform" method="post">
            <div class="List">
                ...
                <?php if(is_array($article_list)): foreach($article_list as $k=>$vo): ?><div class="ProList">
                        <ul>
                            ...
                            <li class="Name"><?php echo (str_cut($vo["title"],80)); ?></li>
                            <li class="Property"><?php echo (date("Y-m-d",$vo["inputtime"])); ?></li>
                            <li class="S_Order"><font color=red><?php echo (date("Y-m-d",$vo["updatetime"])); ?></font></li>
                            <li class="Status"><?php echo ($vo[status]==1?'正常':'<font color=red>已删除</font>'); ?></li>
                            <li class="Oper">
                                <div>
                                    <a href="<?php echo U('Article/status',array('id'=>$vo['id'],'status'=>$vo['status']));?>">还原</a>
                                    <span>|</span>
                                    <a href="<?php echo U('Article/delete',array('id'=>$vo['id']));?>" onclick="return confirm('确认是否永久删除?')">永久删除</a>
                                </div>
                            </li>
                        </ul>
