						...
						<div class="cpxqtagContent selectTag" id=cpxqtagContent0 style="display:block;">
							<ul>
								...
								<li class="input">
									<select name="catid">
										<option>选择商品</option>
										<?php if(is_array($goods_list)): foreach($goods_list as $k=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" <?php if($detail['goodsid'] == $vo['id']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; ?>
									</select>
								</li>
							</ul>
							...
							<ul>
								...
								<li class="input"><script type="text/plain" id="editor" style="width: 100%;height: 350px;"><?php echo ($detail["content"]); ?></script></li>
							</ul>
							<style>.icheckbox_square-aero {margin: 1.5px 0px;}</style>
							<ul>
								<li class="Name">是否发布</li>
								<li class="input">
									<div class="radio"><span><input type="radio" name="status" value="1" <?php if($detail['status']): ?>checked="checked"<?php endif; ?>></span><label for="status1">已发布</label></div>
